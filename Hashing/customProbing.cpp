...
void HashTable::insertItem(string key, int data)
{
    ...
    if(table[hashValue1].data!= 0)
    {
        ...
        while(1)
        {
            int newIn= (hashValue1+ 7*i*hashValue2 + 5* i*i)%capacity;
...
void HashTable::removeItem(string key)
{
    ...
    int hashValue2= auxHash(key);
    int i=0;
    while(table[(hashValue1+ 7*i*hashValue2 + 5* i*i)%capacity].key.compare(key) !=0)
    {
        if(table[(hashValue1+ 7*i*hashValue2 + 5* i*i)%capacity].data == 0)
        {
            cout<< "Not found\n";
            return;
        }
    ...
    table.erase(table.begin()+ (hashValue1+ 7*i*hashValue2 + 5* i*i)%capacity);
